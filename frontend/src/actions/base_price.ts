...
import { PropertyBasePrice } from "@/types/api";

export async function getBasePriceHistory(propertyId: string): Promise<PropertyBasePrice[]> {
    try {
        const res = await serverApi(`/properties/${propertyId}/base-price/history`);
        if (!res.ok) return [];
        return res.json();
    } catch {
        return [];
